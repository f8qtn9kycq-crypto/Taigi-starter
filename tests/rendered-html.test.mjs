...
test("ships the first-time Taigi landing content and Vercel feedback path", async () => {
  ...
  assert.match(stagePanel, /const revealAnswer = \(\) => \{[\s\S]*onRecallRevealed\(\)/);
  ...
  assert.match(stagePanel, /lessonStage\.id === "recall" && showAnswer/);
  assert.match(stagePanel, /lessonStage\.id === "recall" && showAnswer[\s\S]*className="recall-use-reveal"/);
  assert.match(stagePanel, /className="recall-use-reveal"[\s\S]*stage="use"/);
  assert.match(stagePanel, /hideNextStageNavigation=\{lessonStage\.id === "recall" && showAnswer\}/);
  assert.doesNotMatch(stagePanel, /lessonStage\.id === "recall" && showAnswer && <button[^>]*onClick=\{onAdvance\}/);
